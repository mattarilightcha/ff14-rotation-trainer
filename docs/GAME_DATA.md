...
- アクションのアイコンは、**スクウェア・エニックスのファンキットのアイコン**（`public/fankit/battle-pve/`。ホットバーの枠・光沢・影が付いた 76×76）を使う。英語名で突き合わせ、見つからないものだけ第 1 層の PNG を使う（`public/fankit/README.md`）。
- ステータス・ジョブのアイコンは、第 1 層の PNG を使う。**抽出データにアイコンがない場合だけ**、文字と CSS の仮アイコン（SCREENS.md §3）を使う。
...
- 音声は引き続き使わない（モックの効果音はブラウザで合成）。
- **ジョブゲージ**は、抽出した ULD（`job-gauges.json`: パーツの切り出し座標・ノードの配置）とテクスチャ（`public/icons/job-gauges/textures/*.png`）で、ゲームと同じ部品の組み合わせを画面に組み立てる（`mock/uld.js`）。テクスチャは切り分けず、アトラスのまま CSS の背景位置で切り出す。高解像度版（`scale: 2`）は座標を 1/2 にして使う。
  - 兄弟ノードは ULD で先に書かれたものほど手前に描く（剣気ゲージの数字のノードが先頭にあることから判断）。回転は度。加算色（`add`）は SVG の色行列で再現する。
  - どのノードをいつ出すか（点灯・塗りの幅）は、ゲームの状態からモックが決める（`mock/gauge.js`）。光り方の動きは ULD のタイムラインを抽出していないため仮（UI-01）。
...
| DATA-04 | `actionProcStatusId`（ActionProcStatus の行が指すステータス）を抽出に追加済み。再抽出で `actions.json` に入り、光る条件（SPEC §6.1a）をデータだけで判定できる。それまでモックは行番号→ステータスの対応表を使う |
| LEGAL-01 | 抽出したアイコン・文言を公開サイトで使う条件の確認 |
| UI-01 | ジョブゲージの動き（点いたとき・減ったときの光り方）。ULD のタイムライン（キーフレーム）を抽出すれば、ゲームと同じ動きにできる。いまは ULD の各ノードの初期値（拡大率・透明度）を手がかりにした仮の動き |
| UI-02 | 画像ノードのラップ方式（ノードがパーツより小さいとき切り取るか、伸ばすか）。抽出データにないため、モックは切り取りに統一（剣気ゲージの柄は 116 → 106 で右端が少し欠ける） |
| UI-03 | クリッピングマスクのノード（種類 10。忍者・ピクトマンサーのゲージで使う）がどのパーツを使うか。抽出データに入っていない |
| UI-04 | ゲージの数字（テキストノード）の字形・大きさ・色。抽出データにないため、モックは近い見た目の書体で代用 |
