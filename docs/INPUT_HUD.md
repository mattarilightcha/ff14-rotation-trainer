...
モックの練習場は 3D の斜め見下ろしで、カメラを回せる（真上の 2D は北が上で固定）。操作は PC 版 FF14 に合わせる。

| 入力 | 動き |
|------|------|
| マウスの左ドラッグ・右ドラッグ | カメラを回す（左右・上下） |
| マウスの左右のボタンを同時に押す | カメラの向きへ前進（押したままドラッグすると曲がれる） |
| マウスのホイール | 近づける・離す |
| KEYBIND.DAT のカメラ操作 | `CAMERA_LEFT` / `CAMERA_RIGHT`（左右に回す）、`CAM_TILT_UP` / `CAM_TILT_DOWN`（上下に傾ける）、`CAMERA_ZOOMIN` / `CAMERA_ZOOMOUT`、`CAMERA_RESET`（自分の後ろへ）。修飾キー込みで一致したときだけ（CONFIG_FORMAT §6） |
| パッドの右スティック | カメラを回す |

- 移動キーの「前」はカメラの向き（画面の奥）。カメラを回すと移動の向きも変わる（GAME-57）。
- 矢印キーでの移動（設定）は、カメラ操作に割り当てられた矢印を除く（サンプルは ←→ がカメラ）。
- 回す速さ・左右 / 上下の反転は設定で変えられる（DESIGN-05）。
...
|---------|----------------|-----------------|------|
| `HOTBAR.DAT` | ジョブごとのホットバー 1〜10 とクロスホットバーの中身（アクション ID） | ボタンの並び | ✅ 解読済み（[CONFIG_FORMAT.md §2](./CONFIG_FORMAT.md#2-hotbardatcfg-01-解読済み)） |
| `KEYBIND.DAT` | ホットバーの各スロットに割り当てたキー | キーボードの割り当て | TODO(CFG-02) |
| `MACRO.DAT`・`MACROSYS.DAT` | マクロの中の `/ac` の行（アクション ID・対象・待ち時間だけ） | マクロのボタン | ✅（[CONFIG_FORMAT.md §9](./CONFIG_FORMAT.md#9-macrodatmacrosysdatマクロ形式は実物-1-つで確認)） |
...
**読まないファイル:** `UISAVE.DAT`（他のプレイヤーの名前などが入っているため。[CONFIG_FORMAT.md §3.1](./CONFIG_FORMAT.md#31-個人情報についての注意重要)）、マクロの題名と文（読むのはアクションの行だけ）、チャットのログ、ギアセット、フレンドリストなど、練習に関係ないファイルは読まない。アプリは上の表のファイル名だけを受け付け、それ以外は選ばれても無視する。
...
- ゲームでは、ホットバーごとに「全ジョブで共有する」かどうかを設定できる。共有にしたバーの中身は、HOTBAR.DAT のセット 0 に入る（[CONFIG_FORMAT.md §2.2](./CONFIG_FORMAT.md#22-セット番号)）。
- どのバーが共有かの設定の保存場所は未解読（CFG-08）。それまでは、**ジョブのセットのバーに中身があればジョブ専用、なければ共有**と推定する。読み込み画面（S8）と練習画面で、バーごとに切り替えられるようにする。
- 共有のバーには他のジョブのアクションが残っていることがある。練習するジョブで使えないアクションは灰色で表示する。

### 5.3 アクション ID の対応

- ホットバーのスロットに入っているのがアクションなら、その ID を第 1 層（`actions.json`）の ID として引く（同じゲームの ID なので、そのまま対応できる見込み。TODO(CFG-01) で確認）。
- マクロのスロットは、MACRO.DAT（共通マクロは MACROSYS.DAT）を読み込むと、中の `/ac` の行のアクションを使うボタンになる（右上に「M」）。読み込んでいない・このジョブのアクションの行がないときは灰色。
  - 動き: 待ち（`/wait`・`<wait.N>`）のない行は上から順に試し、最初に使えたもので決まる（ゲームでも残りの行は硬直で失敗する）。待ちのある行は、その時間が来てから試す。実行中に別のマクロを押すと前のマクロは止まる（仮 GAME-66）。
  - マクロのアクションは**先行入力されない**（GAME-05）。
  - 対象: `<me>`・`<1>` = 自分、`<2>`〜`<8>` = 相方、`<mo>` = マウスを乗せている人物（パーティリストの行・練習場の人物。乗せていなければその行は失敗し、次の行へ）、`<t>` = いまのターゲット、`<tt>` = ターゲットのターゲット。ターゲットの決め方は SPEC §10 DESIGN-10。地面指定の技は `<me>` = 足元、`<t>` = 敵の足元、`<mo>`・`<gtoff>` = マウスの位置に、ターゲットサークルを出さずに置く。
- アクション以外（アイテム・一般アクション・空きなど）のスロットは「練習では使えないマス」として灰色で表示し、押しても何も起きない。
...
### 5.4 アクションの変化（別ボタン化）の設定

ゲームには、居合術 → 彼岸花・天下五剣…のように**ボタンのアクションが変化する動き**を止め、変化先を別のボタンとして置けるようにする設定がある。この設定でホットバーの中身とボタンの動きが変わるため、取り込む必要がある。

| 項目 | 設計 |
|------|------|
| 変化のグループ | 第 1 層の `replacesAction` から作る（例: 居合術 → 彼岸花 / 天下五剣 / 乱れ雪月花 / 天道五剣 / 天道雪月花、燕返し → 返し五剣 / 返し雪月花 / 返し波切 / 天道返し…、奥義波切 → 返し波切） |
| 設定の持ち方 | グループごとに「変化する（既定）/ 変化させない」を `InputProfile.actionChange[baseActionId]` に持つ |
| 取り込み | ① 設定ファイル上の保存場所が分かれば、そこから読む（CFG-11）。② 分かるまでは、**変化先のアクションがホットバーに直接置かれていれば「変化させない」**と推定する（変化先は通常ホットバーに登録できない、とツールチップにあるため）。③ 画面で手動で切り替えられる |
| 「変化させない」ときの表示 | 元のボタンのアイコンは変えない。変化先はそれぞれ独立したボタンとして、置かれたスロットに表示する。発動条件を満たしたボタンだけが使える（光る） |
| 未確認 | 「変化させない」ときに元のボタン（居合術など）を押すとどうなるか（CFG-11） |
...
| ホットバー 1〜10 | ⑨ スキルボタン | ✅ |
| クロスホットバー | ⑨（パッドの配置） | ✅ |
| ジョブゲージ | ④（ゲームの ULD とテクスチャで組み立てる。ADDON.DAT の配置は大きさで特定: CONFIG_FORMAT §7） | ✅ |
| バフ・デバフ表示 | ②③ | ✅ |
| キャストバー | ⑦（詠唱バー） | ✅ |
| ターゲット情報（まとめ / 分割: HP・キャストバー・ステータス） | ②（敵の欄） | ✅ |
...
| ID | 内容 | 確かめ方 |
|----|------|---------|
| CFG-01 | `HOTBAR.DAT` の形式 | ✅ サンプルで解読済み（CONFIG_FORMAT §2）。残りは CFG-08〜10 |
...
| CFG-08 | 共有にしているバーの設定の保存場所 | `tools/cfg-diff` で設定変更前後を比べる（CONFIG_FORMAT §4） |
| CFG-11 | アクションの変化の設定の保存場所と、元ボタンの動き | 同上＋ゲーム内で確認 |
| CFG-17 | ステータス情報・ターゲット情報を「分割」表示にしているかの保存場所 | ADDON.DAT には両方の位置がある。キャラクターコンフィグの差分で探す（CONFIG_FORMAT §4） |
| CFG-18 | HUD の部品のゲーム内の名前（HUD シート） | 抽出ツールで HUD シートを書き出す。モックは日本語名を仮に付けている |
| CFG-15 | 移動キー（KEYBIND.DAT の `MOVE_FORE` / `MOVE_BACK` / `MOVE_LEFT` / `MOVE_RIGHT` / `JUMP`）の意味。モックは「前」をカメラの向きにしている（レガシーに近い） | ゲームの操作方式（スタンダード / レガシー）がどのファイルに保存されるかを確認（CONFIG_FORMAT §4 の差分の取り方） |
| CFG-19 | KEYBIND.DAT のカメラ操作のうち、`CAMERA_FORWARD`（サンプルは左 Ctrl）・`CAMERA_SAVE` / `CAMERA_LOAD`・`CAMERA_MODE` の意味 | ゲームのキー設定画面の項目と照らす。モックは使っていない |
| CFG-16 | FFXIV.cfg の `DeadArea`（サンプル 0.55）がスティックの遊びか | パッド設定の画面と照らして確認。モックは初期値として使い、設定で変えられる |
