...
- GCD 使用時: `gcd.lastStartAt = t`, `gcd.readyAt = t + R`, `lock.until = t + max(L, C + L_after_cast)`（詠唱後ロックの扱いは TODO(GAME-04, 07)）。
...
flowchart TD
    ...
    F -- Yes --> H[実行 §4.3]
    F -- No --> Q{実行可能になるまでの残り時間が<br/>先行入力受付時間以内?}
    Q -- Yes --> I[queue に格納（既存キューは上書き）]
    Q -- No --> X4[拒否: ON_COOLDOWN / ANIMATION_LOCKED]
```
...
- キュー可能な条件（GCD と oGCD でキューの挙動が異なるか等）は TODO(GAME-05)。MVP は「GCD・oGCD とも同じ規則」で実装し、データで差し替え可能にする。
...
4. ロック設定。詠唱ありなら `pendingEffects` に効果を積み、詠唱完了時に 5〜6 を行う。
...
| ID | 内容 |
|----|------|
| GAME-04 / 07 | 詠唱完了後に追加のロックがあるか、詠唱中のロックとの関係 |
| GAME-05 | 先行入力受付時間と、GCD/oGCD でのキュー挙動の違い |
