...
| `src/data/ffxiv/jobs.json` | 戦闘クラス・ジョブ（ロール、親クラス、LB、使えるアクションの id 一覧、関わるステータスの id 一覧） | 32 |
| `src/data/ffxiv/actions.json` | PvE のプレイヤーアクション（ロールアクション・LB・コンボ派生・忍術などの置き換え先を含む） | 1,055 |
| `src/data/ffxiv/statuses.json` | ジョブに関わるステータス（自分・味方へのバフ、敵へのデバフ・DoT、LB の効果。PvP 版も含む） | 1,084 |
| `src/data/ffxiv/statuses-all.json` | Status シートの全ステータス（名前かアイコンがあるもの。食事・薬・FC バフ・敵やギミックのものも） | 4,812 |
...
| `public/icons/statuses/<アイコン番号6桁>.png` | 全ステータスのアイコン（スタック 2 以上の絵も含む） | 4,641 |
...
アイコンは **Status シートの全行**（4,812 件）の分を `public/icons/statuses/` に出している。
スタックのあるステータス（414 件）は、ゲームがスタック数ごとに別の絵（`icon + スタック数 - 1`）を出すので、
その絵も全部出して `stackIconPaths`（スタック 2〜最大の順）に入れている。
画像がゲームに無いのは 1 枚だけ（Status 701 戦闘能力低下 の 215049）。

`statuses.json` は、次のどれかでジョブに結びつくステータスだけを入れた、アプリ向けの軽い版。`jobs` にそのジョブ、
`jobs.json` の各ジョブの `statusIds` に逆引きがある。

1. アクションの `StatusGainSelf`（自分に付く）と、二次コストがステータスを指すもの
   （型 32 走竜・猛虎・印 / 35 忍隠 / 46 黒魔紋・天道・活殺自在 / 127 死の供物。ほかの型の値はゲージ量・回数・フラグで、
   ステータス番号と偶然重なるだけなので使わない。以前はこれを区別しておらず、石化 #1・スタン #2 などが混ざっていた）
2. `Status.ClassJobCategory` が一部のジョブだけを指すもの（ディア → 白魔道士、エウクラシア・ドシス → 賢者 など。
   `StatusGainSelf` はほとんどのアクションで空なので、ジョブのバフ・敵へのデバフの大半はここで拾える）
3. ジョブのアクションと日本語名が同じもの（ランパート・ファイト・オア・フライトなど、古い行は全クラス扱いになっているため）

注意:
- ステータス名はアクション名と違うことがある（サーチングライト → シアリングライト、スターリーミューズ → イマジンスカイ）
- 同じ名前のステータスが複数あることがある（ディア #1871・#2035 など。PvP 版や旧版）。Status シートに PvP かどうかの列は無い
- LB の効果（シールドウォール・原初の大地など）はパーティー全員に付くので、全ジョブに入る
- どのアクションがどのステータスを敵に付けるか（DoT の対応）は Excel シートに無い。名前で対応させる
...
- ステータス: 全ステータスのアイコン（スタック別を含む）がそろうこと、ジョブに結びつかないものが statuses.json に無いこと、無関係のもの（石化 #1 など）が混ざっていないこと、ジョブごとの代表的なステータス（ディア・彼岸花・ハイサンダーなど 33 件）があること
