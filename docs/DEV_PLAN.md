...
| PR-1 | プロジェクト雛形と CI / Pages デプロイ | Vite + React + TS（strict）、Vitest、ESLint、Prettier、`base` 設定、ハッシュルーター骨組み、非公式表記フッター、GitHub Actions（CI と Pages デプロイ） | スモークテスト 1 本、S-01, S-04 | PR-0 |
...
| PR-16 | 公開 v0.1 | README（使い方・非公式表記・出典）、リリースノート | M-09 | 全部 |

## 3. 依存関係

```mermaid
flowchart TD
    P0[PR-0 設計書] --> P1[PR-1 雛形/CI/Pages]
...
| マイルストーン | 含む PR | できること |
|---------------|--------|-----------|
| M1: 動く骨組み | PR-1〜5 | テストジョブでエンジンがテスト上動く。Pages に雛形が公開される |
...
> 公開ページ（GitHub Pages）は PR-1 から自動デプロイするが、侍データが `verified` になるまでは「データ未整備」表示となり練習は開始できない（テストジョブは開発ビルドのみ）。
